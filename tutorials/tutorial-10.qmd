...
format:
    unilur-question-html:
        output-file: tutorial-10.html
        embed-resources: true
    unilur-solution-html:
        output-file: tutorial-10sol.html
        embed-resources: true
---
...
```{r fig.width=7, fig.height=12, out.width="100%"}
data(bomregions)
...
```{r fig.width=8, fig.height=6, out.width="100%", message=FALSE, warning=FALSE}
...
```{r fig.width=12, fig.height=8, out.width="100%", message=FALSE, warning=FALSE}
US_unemp <- US_unemp %>%
...
```{r  fig.width=12, fig.height=8, out.width="100%", warning=FALSE}
...
a. Make a line plot of count by year. Do you agree that it looks periodic?
...
b. Create two new variables by rounding the year into a decade, and the remainder into a year in the decade.
...
c. Add a vertical line every 10 years, to your plot of counts over time. If you start from 1928, the location of the first peak, you can check the  peak locations in  subsequent years. Is the peak roughly every 10 years?
...
d. Cut the series into decades, and make overlaid line plots of count vs year in decade, using decade as the group variable. This is like looking at seasonality, like we might look at seasonal patterns in a year by examining the months. If the series is cyclic, particularly with peaks every 10 years, the peaks should line up. Do they?
...
```{r fig.width=14, fig.height=2, out.width="100%", warning=FALSE}
...
```{r fig.width=8, fig.height=5, out.width="100%", message=FALSE}
ggplot(heights,
       aes(x = year,
           y = height_cm)) + 
