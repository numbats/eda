...
e. If we throw all the neighbourhoods in together to analyse price and property characteristics, what paradox might we encounter?
...
a. Compute several of the scagnostics (monotonic, outlying, clumpy2) for the first 5 variables of variables, except for `name`. 
...
b. Sort the scagnostics, separately by the values on (i) monotonic (ii) outlying (iii) clumpy2, and plot the pair of variables with the highest values on each. 
